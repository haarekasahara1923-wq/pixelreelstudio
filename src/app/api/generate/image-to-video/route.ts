...
export async function POST(req: Request) {
  try {
    ...
    if (isMock) {
      ...
      try {
        // Submit task to Fal Image-to-Video Queue
        ...
        if (!queueResponse.ok) {
          ...
          throw new Error(`Fal.ai Image-to-Video Queue failed: ${errText}`);
        }
        ...
        console.error("Image-to-video generation failed. Refunding credits...", genError);
        // Refund credits
        await db.update(users).set({ credits: user.credits }).where(eq(users.id, userId));
        return NextResponse.json(
          { error: "Image-to-Video generation failed. Credits refunded." },
          { status: 500 }
        );
      }
    }
    ...
    console.error("Image-to-Video generation API error:", error);
    return NextResponse.json(
